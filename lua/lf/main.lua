...
function Lf:__set_cmd_wrapper()
	...
	if self.cfg.focus_on_open and vim.fs.dirname(self.curfile) == self.term.dir then
		open_on = self.curfile
	end

	-- command lf -command '$printf $id > '"$fid"'' -last-dir-path="$tmp" "$@"
	self.term.cmd = ([[%s -command='$printf $id > %s' -last-dir-path='%s' -selection-path='%s' '%s']]):format(
		self.term.cmd,
		self.tmp_id,
		self.tmp_lastdir,
		self.tmp_sel,
...
function Lf:__on_open(term)
	...
	if self.cfg.mappings then
		if self.cfg.escape_quit then
			vim.keymap.set("t", "<Esc>", "<Cmd>q<CR>", { buffer = self.bufnr, desc = "Exit Lf" })
		end
...
function Lf:__callback(term)
	...
	if (self.action == "cd" or self.action == "lcd") and uv.fs_stat(self.tmp_lastdir) then
	...
	for _, arg in ipairs(fn.argv()) do
		...
		if api.nvim_buf_is_loaded(fn.bufnr(arg)) then
			if realpath then
				...
				api.nvim_buf_delete(fn.bufnr(arg), { force = false })
			end
		end
	end
	self.arglist = args
