...
local function become_dir_fman(bufnr)
    ...
    -- Ensure we resolve the path fully
    bufname = fn.fnamemodify(bufname, ":p")
    -- Remove trailing slash for stat if present (except root)
    if bufname:sub(-1) == "/" and #bufname > 1 then
        bufname = bufname:sub(1, -2)
    end
...
local function setup_autocmds()
	...
	if Config.data.default_file_manager then
		...
		if vim.g.loaded_netrwPlugin ~= 1 and not Config.data.disable_netrw_warning then
			api.nvim_create_autocmd("FileType", {
				...
				callback = function()
					utils.warn(
						[[Lf cannot be the default file manager with netrw enabled.]]
			...
			callback = function(a)
				if become_dir_fman(a.buf) then
					vim.schedule(function()
                        -- Ensure we are still in the correct buffer before hijacking
                        if api.nvim_get_current_buf() ~= a.buf then
                            return
                        end
                        
                        local cfg = Config.data
                        -- For hijacking, we force the hijack flag
                        cfg = vim.deepcopy(cfg)
                        cfg.hijack_netrw = true
						require("lf").start(a.file, cfg)
					end)
				end
			end,
		})
